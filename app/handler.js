...
module.exports.processNewProjectPermissions = (date) => {
	return new Promise((resolve, reject) => {
		...
		//get Users
		    Promise.all([rally_utils.getProjectsByCreationDate(workspace_ref,['Name'],date),rally_utils.getIntegrationUsers(workspace_ref,['c_IntegrationRole'])])
				.then((results) => {
...
module.exports.processNewUserPermissions = (date) => {
	return new Promise((resolve, reject) => {
		...
		//Get newly created teams
		//get Users
		    Promise.all([rally_utils.getAllProjects(workspace_ref,['Name']),rally_utils.getIntegrationUsersByCreationDate(workspace_ref,['c_IntegrationRole'],date)])
				.then((results) => {
					log.info('Projects Created: ',results[0].TotalResultCount, 'Users that will be updated: ',results[1].TotalResultCount );
					var data_array = [];
					if(results[0].TotalResultCount > 0 && results[1].TotalResultCount > 0){
...
const runSerialPermissions = (data_array,workspace_ref) => {
  ...
  _.each(data_array,function(data){
    result = result.then(() => rally_utils.createArtifact(workspace_ref, 'ProjectPermission',  true, data).then((permission) => { 
        log.info('Permissions Updated: ',permission.Object.Name, permission.Object.User._refObjectName, permission.Object.Project._refObjectName );
    }));
  })
  return result;
}